...
import com.handmark.pulltorefresh.library.internal.BDPRotateLoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    // Fixme: 设为public是为了从包外部可以继承实现其他类型的ListView
	...
	public static enum AnimationStyle {
		...
		/**
		 * This is for BDP rainbow-style animation. Current practice is to set drawables according
         * to the pulling-down distance. Allows you to use any drawable.
		 */
		RAINBOW_ROTATE;
		...
		LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
			switch (this) {
                ...
                case RAINBOW_ROTATE:
                    return new BDPRotateLoadingLayout(context, mode, scrollDirection, attrs);
			}
		}
	}
    ...
    public void DoRefresh(){
        // 总是因为refreshing状态的ui高度跟待刷新的ui高度不一致，导致调用此方法时，顶部会有空余，所以只好在layout里定死高度
        if (null != mOnRefreshListener) {
            setState(State.REFRESHING, true);
            mOnRefreshListener.onRefresh(this);
            return;
	...
	/**
	 * 设置顶部和底部刷新view的文本字体颜色
	 * @param color
     */
	public void setHeaderAndFooterTextColor(int color) {
		mHeaderLayout.setTextLabelColor(color);
		mFooterLayout.setTextLabelColor(color);
	}
